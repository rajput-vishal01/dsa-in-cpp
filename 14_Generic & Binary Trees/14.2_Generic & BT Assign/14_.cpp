...
class Solution {
  public:
     Node* makeNodeToParentMappingAndFindTargetNode(Node* root,unordered_map<Node*,Node*>&mp, int target) {
        queue<Node*>q;
        Node* targetNode =0;
        q.push(root);
        mp[root]=0;
    ...
    int minTime(Node* root, int target) {
        unordered_map<Node*,Node*>mp; //stores mapping : Node->its parent
        Node* targetNode=makeNodeToParentMappingAndFindTargetNode(root,mp,target);
        
        int ans = burnTheTree(targetNode,mp);
        return ans;
    }
};